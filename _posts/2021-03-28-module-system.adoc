...
This post assumes that you can write a "`hello world`" Rust program. It's a rather long read, so get comfortable, maybe with a cup of tea, hot chocolate, or whatever your heart desires 😊
...
Visibility means that we are principally allowed to use an item somewhere. It doesn't mean that the item is available, i.e. _in scope_, so we might still have to import it.
...
Discussion on Reddit. You can also open an issue in the https://github.com/Aloso/aloso.github.io/issues[issue tracker].
