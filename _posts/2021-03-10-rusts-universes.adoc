...
How does that work? Let's start with the obvious ones: `a` and `h` are functions, and `c` is a local variable, so they must be in the value namespace. `j` is used as a function argument, so it's also a value. `C` and `D` are used in type positions, so they're types. `I` is used as a generic argument, so it's also a type.
